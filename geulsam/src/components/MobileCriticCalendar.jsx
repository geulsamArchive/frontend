...
const MobileCriticCalendar = ({ criticData, onDataUpdate }) => {
      ...
      if (error.response && error.response.status === 403) {
        ...
        try {
          ...
          if (tokenResponse.status === 200) {
            ...
            const newRefreshToken = tokenResponse.headers.refreshtoken.replace(
              'Bearer ',
              ''
            );
          ...
          try {
            ...
            if (tokenResponse.status === 200) {
              ...
              const newRefreshToken =
                tokenResponse.headers.refreshtoken.replace('Bearer ', '');
              localStorage.setItem('refresh', newRefreshToken);
              alert('삭제가 완료되었습니다!');
              onDataUpdate();
            }
  ...
  return (
    <MobileCalendarContainer>
      <BookTitle>금학기 합평 신청 및 승인 확인</BookTitle>
      <CriticContainer>
        <CriticTop>
          {monthIdx > 0 ? (
            <svg
            ...
            >
              <path
                d="M9.35 1.1333L1.98334 8.49997L9.35 15.8666"
                stroke="#575655"
          ...
          {monthIdx < criticData.length - 1 ? (
            ...
            >
              <path
                d="M2.18332 1.63379L9.54999 9.00046L2.18332 16.3671"
                stroke="#575655"
                stroke-width="2.26667"
                stroke-linecap="round"
              />
                    ...
                    <Conditions>
                      {author.condition === 'UNFIXED' ? (
                        <svg
                          ...
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          onClick={() => handleDelete(author.criticismAuthorId)}
                        >
                          <path
                            d="M7.48286 7.16895L14.1171 13.4109"
                            stroke="#81807F"
                            stroke-width="1.91314"
                            stroke-linecap="round"
                          />
                          <path
                            d="M14.1171 7.16895L7.48286 13.4109"
                            stroke="#81807F"
                            stroke-width="1.91314"
                            stroke-linecap="round"
                          />
